...
//  Company 클래스의 field 의 요구사항에 모두 맞으면
 ...
 //    Map<String, CommonMember> mbr_map 에 저장시켜주는 메소드 ==
	...
	public void register(Scanner sc, List<CommonMember> mbr_list, Map<String, CommonMember> mbr_Map) {
		...
		do {
			...
			for(int i=0; i<mbr_list.size(); i++) {
				if(mbr_list.get(i).getType()==2 && id.equals(mbr_list.get(i).getId())) {
		...
		cp.setType(2);
		mbr_list.add(cp);
		mbr_Map.put(cp.getId()+"_cp", cp); // "_cp" 를 추가한 이유는 구직자와 회사의 id 값이 동일할경우 덮어쓰기를 방지하기 위해서 
	...
	public CommonMember login(Scanner sc, Map<String, CommonMember> mbr_map) {
		...
		CommonMember mbr = mbr_map.get(id+"_cp"); 
	...
	}// end of public Company login(Scanner sc, Map<String, CommonMember> mbr_map)---------------------


	// === 구인회사 전용메뉴 ===
	@Override
	public void cp_menu(Scanner sc,
						Company login_cp, 
		...
		do {
	        ...
	        switch (str_menuno) {
				...
				case "3": // 모든구직자 조회 
					view_all_gujikja_info(mbr_list);
					break;
					
				case "4": // 구직자검색하기 
					search_gujikja(sc, mbr_list);
					break;	
					
				case "5": // 채용공고입력하기 
					register_recruit(sc, login_cp, recruit_list);
					break;
					
				case "6": // 우리회사 채용공고 조회 
				    view_recruit_mycompany(login_cp,recruit_list);
					break;
	...
	}// end of public void cp_menu(Scanner sc, Company_imple login_cp, CommonMember_abstract[] cmbr_arr, Recruit_imple[] rc_arr, RecruitApply_imple[] rcApply_arr)-----------------------
    ...
    */
	...
	// === 모든구직자 조회 ===
	private void view_all_gujikja_info(List<CommonMember> mbr_list) {
		// >>> 리스트에 저장된 데이터를 쉽게 처리할수 있게 해주는 기능을 가진 stream 변경하여 처리해보기 <<<
		/*
        ...
        ---------------------------------------------------
    *//*
		boolean is_exists_gujikja = false;
		for(CommonMember mbr :mbr_list)  {
			if(mbr.getType() == 1) {
				is_exists_gujikja = true;
				break;
			}
		}// end of for----------------------------
		*/// 또는 
		boolean is_exists_gujikja = mbr_list.stream().anyMatch(mbr -> mbr.getType() == 1);
		
		if(!is_exists_gujikja) {
	...
	}// end of private void view_all_gujikja_info(CommonMember_abstract[] cmbr_arr)------
	
	
	// === 구직자검색하기 ===
	private void search_gujikja(Scanner sc, List<CommonMember> mbr_list) {
		...
		do {
			...
			switch (str_menuno) {
				case "1": // 성별검색
					search_gender_gujikja(sc, mbr_list);  
					break;
					
				case "2": // 연령대검색
					search_ageline_gujikja(sc, mbr_list);
					break;
					
				case "3": // 구직자 연령대 및 성별검색
					search_ageline_gender_gujikja(sc, mbr_list);
					break;
	...
	}// end of private void search_gujikja(Scanner sc, CommonMember_abstract[] cmbr_arr)------------


	// === 성별검색 ===
	private void search_gender_gujikja(Scanner sc, List<CommonMember> mbr_list) {
		...
		if(!is_exists_gujikja) {
			...
			for(CommonMember mbr : mbr_list) {
				if(mbr.getType()==1  
				   && (((Gujikja) mbr).gender().equals(input_gender)))
				  {
					isFind = true;
					sb.append(mbr.getInfo() + "\n");
				  }
	...
	}// end of private void search_gender_gujikja(Scanner sc, CommonMember_abstract[] cmbr_arr)------
	

	// === 연령대검색 ===
    private void search_ageline_gujikja(Scanner sc, List<CommonMember> mbr_list) {
		...
		if(!is_exists_gujikja) {
			...
			for(CommonMember mbr : mbr_list) {
				try {
					if(mbr.getType()==1 
					   && MyUtil.age(((Gujikja)mbr).getJubun())/10*10 == Integer.parseInt(input_ageline) 
					  ) {
						  isFind = true;
						  sb.append(mbr.getInfo() + "\n");
					    }
	...
	}// end of private void search_ageline_gujikja(Scanner sc, CommonMember_abstract[] cmbr_arr)-----
    
    
    // 구직자 연령대 및 성별검색
 	private void search_ageline_gender_gujikja(Scanner sc, List<CommonMember> mbr_list) {
 	...
 	}// end of private void search_ageline_gender_gujikja(Scanner sc, CommonMember_abstract[] cmbr_arr)-----
    
	
    // === 채용공고입력하기 === 
 	private void register_recruit(Scanner sc, Company login_cp,List<Recruit> recruit_list) {
    ...
    // === 우리회사 채용공고 조회(채용마감일자가 지난것이 아니라 진행중인 것만 조회한다). ===
 	private void view_recruit_mycompany(Company login_cp, List<Recruit> recruit_list) {
 		
 		boolean is_existence = false;
 		
 		for(Recruit recruit : recruit_list)  {
 			
 			if( login_cp.getId().equals(recruit.getCp().getId()) ) {
 				
 				String str_finish_day = recruit.getFinish_day();
 			...
 			*/
 			//  또는	
 				...
 				if(n==0 || n==1) { // 채용마감일자가 오늘 이거나 내일 이후인 것.
 					is_existence = true;
 					System.out.println(recruit.recruit_info());
 				}
 			}
 		}// end of for--------------------
    ...
    // === 우리회사 채용공고 지원자 조회 ===
 	private void view_gujikja_my_recruitApply(Company login_cp, List<RecruitApply> rcapply_list) {
	 		...
	 		for(RecruitApply rcapply : rcapply_list) {
	 			...
	 			if( login_cp.getId().equals(rcapply.getRc().getCp().getId()) ) {
	 				...
	 				sb.append(rcapply.getRc().getRecruit_no()+"\t"+
	 						...
	 						rcapply.getGu().gender()+"\t"+
	 						  MyUtil.age(rcapply.getGu().getJubun())+"\t"+
