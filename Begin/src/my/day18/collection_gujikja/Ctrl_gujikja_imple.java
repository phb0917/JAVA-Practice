...
public class Ctrl_gujikja_imple implements Ctrl_gujikja {
	...
	@Override
	public void register(Scanner sc, List<CommonMember> mbr_list, Map<String, CommonMember> mbr_Map) {
		
		 // 지금까지 생성된 구직자 회원수가 cmbr_arr.length(==> 정원) 보다 적을 경우에만 신규회원을 받아들인다.  
			...
			do {
				...
				for(int i=0; i<mbr_list.size(); i++) {
					if(mbr_list.get(i).getType() == 1 && userid.equals(mbr_list.get(i).getId())) {
			...
			gu.setType(1);
			mbr_list.add(gu);
			mbr_Map.put(gu.getId()+"_gu", gu);//"_cp" 를 추가한 이유는 구직자와 회사의 id 값이 동일할경우 덮어쓰기를 방지하기 위해서
	...
	// === 구직자 로그인 === //
	@Override
	public CommonMember login(Scanner sc, Map<String, CommonMember> mbr_map) {
		...
		CommonMember mbr = mbr_map.get(id+"_gu"); 
		
		if(mbr == null)
			return null;
	
		if(!passwd.equals(mbr.getPasswd())) {
			return null;
		}
		else {
			return mbr;
	...
	// === 구직자 전용메뉴 ===
	@Override
	public void gu_menu(Scanner sc,
			Gujikja login_gu, 
		...
		do {
	        ...
	        switch (str_menuno) {
				...
				case "3": // 모든구인회사 조회 
					view_all_company_info(mbr_list);
					break;
					
				case "4": // 구인회사검색하기 
					search_company(sc, mbr_list);
					break;	
					
				case "5": // 모든채용공고조회(현재 채용이 진행중인 것만 조회) 
				    view_all_recruit_info(recruit_list);
					break;
					
				case "6": // 채용공고상세보기 
					view_detail_one_recruit_info(sc,recruit_list);
					break;
					
				case "7": // 채용응모하기 
					input_recruitApply(sc, login_gu, recruit_list, rcapply_list);
					break;
    ...
    */
	...
	// === 모든구인회사 조회 === 
	private void view_all_company_info(List<CommonMember> mbr_list) {
		
		boolean is_exists_company = false;
		for(CommonMember mbr: mbr_list) {
			if(mbr.getType() == 2) {
	...
	// === 구인회사검색하기 ===
	private void search_company(Scanner sc, List<CommonMember> mbr_list) {
		...
		do {
			...
			switch (str_menuno) {
				case "1":   // 업종검색
					search_jobtype_company(sc, mbr_list);
					break;
					
				case "2":   // 자본금검색
					search_seedmoney_company(sc, mbr_list);
					break;
	...
	// === 업종검색 === //
	private void search_jobtype_company(Scanner sc,List<CommonMember> mbr_list) {
		...
		for(CommonMember mbr : mbr_list){
			if(mbr.getType()==2 &&
				(((Company) mbr).getJob_type().toLowerCase().contains(job_type_name))) {
				is_existence = true;
				sb.append(mbr.getInfo()+"\n");
			}
		}// end of for-------------------------
	...
	// === 자본금검색 ===	
    private void search_seedmoney_company(Scanner sc, List<CommonMember> mbr_list) {
    	...
    	try {
    		...
    		for(CommonMember mbr: mbr_list) {
    			if(mbr.getType()==2 && 
    				(((Company) mbr).getSeed_money() >= search_seed_money)) {
    				is_existence = true;
    				sb.append(mbr.getInfo()+"\n");
    			}
    		}// end of for-------------------------
    ...
    // === 모든채용공고조회(현재 채용이 진행중인 것만 조회) ===
 	private void view_all_recruit_info(List<Recruit> recruit_list) {
			...
			*/
 			...
 			for(Recruit recruit : recruit_list) {
 				...
 				try {
 					 ...
 					 Date finish_day = sdf.parse(recruit.getFinish_day());
					 ...
					 if(result == -1) { // 채용마감일자가 현재일 보다 이전인 경우 
						 ...
						 sb.append(recruit.recruit_info()+"\n");
					 }
				} catch (ParseException e) { }
 				
 			}// end of for---------------------
 			
 			if(recruit_list.size() == cnt_before) {
 				System.out.println(">> 현재 진행중인 채용공고가 1개도 없습니다.<<\n");
 			}
 			else {
 				System.out.println(sb.toString());
 				System.out.println("[현재 진행중인 채용공고가 "+ (recruit_list.size()- cnt_before)+ "건이 있습니다.]\n");
 			}
 		}
    ...
    // === 채용공고상세보기 ===
 	private void view_detail_one_recruit_info(Scanner sc, List<Recruit> recruit_list) {
 			...
 			try {
				...
				for(Recruit recruit : recruit_list) {
	 				...
	 				if( !finish_day.before(now) &&
	 					str_recruit_no.equals(String.valueOf(recruit.getRecruit_no())) ){          
    ...
    // === 채용응모한것조회(채용마감일자가 오늘 포함 이후인것만 보여지도록 한다) ===
 	private void view_my_recruitApply(Gujikja login_gu,List<RecruitApply> rcapply_list) {
 		...
 		for(RecruitApply rcapply : rcapply_list) {
 			...
 			String finish_day = rcapply.getRc().getFinish_day();
 			
 		    int n = MyUtil.Date_comparison(finish_day, str_today);
 		    //  -1 , 0 , 1 , 2 
 		    
 		    if( (n==0 || n==1) && 
 		    	login_gu.getId().equals( rcapply.getGu().getId()) ) { 
 		    	
 		    	is_existence = true;
 		    	
 		    	sb.append("\n▣ 지원번호 : " +rcapply.getRecruitApply_no()+"\n");
 		    	sb.append(rcapply.getRc().detail_recruit_info());
 		    	sb.append("▣ 지원동기 : " + rcapply.getApply_motive()+"\n");
 		    	
 		    	String register_day = rcapply.getRegister_day();
    ...
    // === 채용응모하기 ===
  	private void input_recruitApply(Scanner sc,Gujikja login_gu,List<Recruit> recruit_list,List<RecruitApply> rcapply_list) {
		...
		int[] recruit_no_arr = new int[recruit_list.size()]; // "채용공고순번" 만 저장하는 용도
		
		for(int i=0; i<recruit_list.size(); i++) {
			
			String finish_day = recruit_list.get(i).getFinish_day();
			...
			if(n==0 || n==1) {
				...
				recruit_no_arr[i] = recruit_list.get(i).getRecruit_no(); // "채용공고순번" 저장시킴
				
				finish_day = recruit_list.get(i).getFinish_day();
				
				sb.append(recruit_list.get(i).getRecruit_no()+"\t"+
						recruit_list.get(i).getCp().getName()+"\t"+
						recruit_list.get(i).getSubject()+"\t"+
						recruit_list.get(i).getWork_type() +"\t\t"+  
						recruit_list.get(i).getCnt()+"\t"+
						  dcf.format(recruit_list.get(i).getYearpay())+"만원\t"+ 
		...
		if(is_existence) {
			...
			for(RecruitApply rcapply : rcapply_list) {
				...
				if( str_recruit_no.equals( String.valueOf(rcapply.getRc().getRecruit_no()) ) ) { 
		...
		/*	
			Recruit rc = null;
			for(recruit recruit : recruit_list) {
				if(str_recruit_no.equals( String.valueOf(recruit.getRecruit_no()) ) ) {
					rc = rc_arr[i];
					break;
				}
			}// end of for-----------------
		...
		*/
		//  또는	
			...
			RecruitApply.count++; // RecruitApply_no 를 매번 1증가 시키기 위해서
    ...
    // === 채용응모한것 수정하기 ===
 	private void update_my_recruitApply(Scanner sc, Gujikja login_gu, List<RecruitApply> rcapply_list) {
		...
		boolean is_others_recruit_no = false;
 		...
 		for(RecruitApply recruit : rcapply_list) {
 			
 			if( str_recruitApply_no.equals(String.valueOf(recruit.getRecruitApply_no()) ) ) {
 				is_exists_recruit_no = true;
 				
 				if( login_gu.getId().equals(recruit.getGu().getId() ) ) {
 				
	 				int n = MyUtil.Date_comparison(recruit.getRc().getFinish_day(), str_today);
	 				// -1, 0, 1, 2
	 				...
	 				if( !(n==0 || n==1) ) { 
	 					...
	 					do {
		 					...
		 					recruit.setApply_motive(sc.nextLine());
	 					} while(!(recruit.getApply_motive() != null));
