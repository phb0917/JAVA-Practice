	...
	public void gu_menu(Scanner sc,  Gujikja login_gu, Company[] cp_arr,Recruit[] rc_arr,RecruitApply[] rcapply_arr) {
		...
		do {
			...
			switch (str_menuno) {
				...
				case "9": //9.채용응모한것수정하기
					update_my_recruitapply(sc,rcapply_arr,login_gu,rc_arr);
					break;
...
//----------------------------------------------------------------
		...
		private void view_my_recruitApply(Gujikja login_gu, RecruitApply[] rcapply_arr) {
			...
			//System.out.println(rcapply_arr[i].getRc().detail_recruit_info());	 이것도 가능 
				is_existence = true;
				sb.append(" >> 지원번호 "+rcapply_arr[i].getrecruitapply_no()+"\n");
		...
		private void input_recruitApply(Scanner sc, Gujikja login_gu, Recruit[] rc_arr, RecruitApply[] rcapply_arr) {
			...
			int[] recruit_no_arr = new int[Recruit.count];
			
			for(int i=0; i<Recruit.count; i++) {
				
			String finish_day =	rc_arr[i].getFinish_day();
			int n =	MyUtil.Date_comparison(finish_day, finish_day);
			// -1 x  0 o 1 o      2 x = 날짜가 아니면
			if (n == 0 || n == 1) {
				is_existence = true;
			...
			if(is_existence) {
				System.out.println("-".repeat(70));
	             ...
	             // 엄정화 기준으로 1001 > 이미 응모한 번호
	             // 엄정화 기준으로 1001 > 응모가 가능한 번호 
	             // 엄정화 기준으로 1001 > 이미 응모한 번호
	             // 9001 >> 존재하지 않는 번호
	             //강아지 
	            ...
	            if(str_recruit_no.equals(String.valueOf(rcapply_arr[i].getRc().getRecruit_no()))) { //입력한 번호가 채용응모 저장소에 존재하는 번호인지 
	            	is_existence = true;
	            	if(login_gu.getId().equals(rcapply_arr[i].getGu().getId())){
	            		//입력한 번호가 채용응모 저장소에 존재하면서 . 이미 응모한 번호라면
	            		System.out.println(">> 입력하신 채용공고번호 " + str_recruit_no + "번은 이미 응모하신 번호입니다" );
	            				return;
	            	}
	             ...
	             // 입력한 번호가 채용응모 저장소에 존재하지 않는 것 이라면  
	             if(!is_existence) {
	            		System.out.println(">> 입력하신 채용공고번호 " + str_recruit_no + "번은 없는 번호입니다<<\n" );
        				return;
	             }
	             // 응모 가능한 번호 이라면 
	             ...
	             rcapply_arr[RecruitApply.count++] = rcapply;
	             System.out.println(">> 채용공고 번호 " + str_recruit_no + "번에 응모 완료 했습니다 ");
			}
			else {
				System.out.println("현재 진행중인 채용공고가 없거나 잘못 입력 하였습니다 ");
			}//end if ~ else 
			
		}//end void
		//9.채용응모한것수정하기
		private void update_my_recruitapply(Scanner sc, RecruitApply[] rcapply_arr, Gujikja login_gu,
				Recruit[] rc_arr) {
			...
			System.out.println("수정할 지원번호를 입력하세요 : ");
			String str_no = sc.nextLine();
			
			for (int i = 0; i < RecruitApply.count; i++) { //채용 지원 횟수만큼 반복문을 돌린다 
				
				int rc_no = rcapply_arr[i].getrecruitapply_no();// 채용지원 배열에 채용번호를 rc_no 값에 넣는다 
				if (str_no.equals(String.valueOf(rc_no))) { // 만약 내가 입력한 번호가 rc_no 값과 같고 

					if (login_gu.equals(rcapply_arr[i].getGu())) { // 로그인한 구직자가 채용 지원 구직자와 같다면 
						int n = MyUtil.Date_comparison(rcapply_arr[i].getRc().getFinish_day(), str_today);// 날짜 비교 채용 마지막 날짜에 따라 -1 0 1 2 로 반환 후 n에 값을 넣음
						if(!(n==0 || n==1)) { // 채용 마감일자가 지났거나 맞지 않는 타입으로 들어왔다면 
							System.out.println(">> 채용마감일자가 지난것이므로 수정이 불가합니다 ");
							break;
						}
						else { // 채용 마감일자가 오늘이거나 내일 이후인것 
							System.out.println("지원동기 변경 내용 :"); 
							String str_no1 = sc.nextLine();         // 값을 받아서 

							rcapply_arr[i].setApply_motive(str_no1); // 그 채용지원 배열에 지원 동기를 변경한다 
							System.out.println( str_no+"번 지원동기 수정 완료 했습니다 ");
			...
			System.out.println(">> 수정 하려고 하시는 채용번호 " + str_no + "번 채용지원은 존재하지 않습니다 ");
