...
public class Recruit {
	...
	public void setwork_type(String work_type) {
		if(work_type != null && !work_type.isBlank()) {
			this.work_type = work_type;}
				...
				System.out.println("[경고] 채용분야을 필수로 입력해야 합니다 ");
			}
	}
	...
	public void setCnt(int cnt) {
		...
		this.cnt = cnt;}
			...
			System.out.println("[경고] 채용인원수는 1명 이상이여야합니다 ");
		}
	}
	...
	public void setYearpay(int yearpay) {
		if(yearpay> 0) {
			...
			this.yearpay = yearpay;}
				...
				System.out.println("[경고] 연봉은 1 이상이여야합니다 ");
			}
	}
	...
	public void setFinish_day(String finish_day) {
		...
		// 채용공고 등록일자 문자열 값을 날짜 타입으로 변환
		Date date_finish_day = sdf.parse(finish_day);
		//채용 마감 일자 문자열값을날짜 타입으로 변환
		
		if(date_finish_day.after(date_register_day)) {
			this.finish_day = finish_day;
		}
		else {
			System.out.println("채용마감일자는 현재일자 보다 이후인 날짜이어야합니다");
		}
		}catch(ParseException e) {
			System.out.println("[경고] 채용마감일자는 달력에 존재 하는 날짜 이여야합니다");
		}
		this.finish_day = finish_day;
	}
