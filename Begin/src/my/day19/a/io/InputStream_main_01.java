...
public class InputStream_main_01 {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(">> 영문자를 입력하세요 [종료하려면 x 를 입력하세요] << ");
		int input = 0;
		int totalByte = 0;
		
		while(true) {
			input = System.in.read(); //abcd 엔터 
			// System.in 은 키보드 라고 생각하면 된다 
			// 키보드에서 입력한 문자열중 글자 1개씩 (char) 만 읽어 들여 char 에 해당하는 int 타입으로 반환해주는 것이다 
			// System.in.read()는 1byte 씩 읽어온다.
			
			//엔터는 \r\n 인데 \r(carriage return) 이 13 이고 \n(new line)이 10인 것이다. 즉, 엔터란 문장 맨 앞으로 가서(\r) 줄 바꿈(\n)을 해라는 것이다.
			if(input != 13 && input !=10) { // 엔터는 제외라는 뜻이다 
